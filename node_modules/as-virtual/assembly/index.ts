...
/**
 * `Virtual<T>` represents a reference to a segment of data
 */
export class Virtual<T> {
    ...
    reinst(data: nonnull<T>, start: i32 = 0, end: i32 = 0): void {
        ...
        if (!end) {
            this.data_ptr = this.data + <usize>start;
            this.byteLength = (changetype<OBJECT>(changetype<usize>(data) - TOTAL_OVERHEAD).rtSize - start);
            this.end = this.byteLength;
        } else {
            const align = ((alignof<T>() as i32) - 1);
            this.data_ptr = this.data + <usize>(start << align);
            this.byteLength = (end - start) << align;
            this.end = end;
        }
     ...
     */
    equals(eq: T): boolean {
        if (this.byteLength !== changetype<OBJECT>(changetype<usize>(eq) - TOTAL_OVERHEAD).rtSize) return false;
        // @ts-ignore
    ...
    copyOut(): T {
        ...
        memory.copy(buffer, this.data_ptr, this.byteLength);
    ...
    /**
     * Retrieve the parent that the Virtual is referencing. Virtual is still attached to parent.
     * @returns T | null
     */
    ...
    /**
     * Destroy Virtual and clean from memory. Reference is broken, so object may be GC'ed now.
    ...
    /**
     * Clear the data being referenced. This will also destroy the Virtual.
     */
    clear(): void {
        // @ts-ignore: __pin is defined
    ...
    merge(item: Virtual<T>): nonnull<T> {
        // @ts-ignore: __new is defined
        const buffer = __new(this.byteLength + item.byteLength, idof<nonnull<T>>());
        memory.copy(buffer, this.data_ptr, this.byteLength);
        memory.copy(buffer + <usize>this.byteLength, item.data_ptr, item.byteLength);
        return changetype<nonnull<T>>(buffer);
    }
    ...
    static mergeVirts<T>(items: Virtual<T>[]): T {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = unchecked(items[i]);
            memory.copy(buffer + <usize>offset, item.data_ptr, item.byteLength);
